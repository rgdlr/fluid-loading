...
describe('skeleton DOM generation without React', () => {
  it('creates skeleton DOM element with bones and shimmer', () => {
    ...
    expect(skeleton.className).toContain('fluid-skeleton');
    expect(skeleton.getAttribute('aria-hidden')).toBe('true');

    const shimmer = skeleton.querySelector('.fluid-shimmer');
    expect(shimmer).not.toBeNull();

    const bones = skeleton.querySelectorAll('.fluid-bone');
    expect(bones).toHaveLength(3);

    expect(bones[0].className).toContain('fluid-bone-rect');
    expect((bones[0] as HTMLElement).style.left).toBe('10px');
    expect((bones[0] as HTMLElement).style.borderRadius).toBe('8px');

    expect(bones[1].className).toContain('fluid-bone-text');
    expect((bones[1] as HTMLElement).style.width).toBe('200px');

    expect(bones[2].className).toContain('fluid-bone-circle');
  ...
  it('creates estimated skeleton element correctly', () => {
    ...
    expect(estimated.className).toContain('fluid-skeleton');
    expect(estimated.className).toContain('fluid-estimated');
    expect(estimated.style.height).toBe('240px');
    expect(estimated.style.width).toBe('320px');
    expect(estimated.querySelector('.fluid-shimmer')).not.toBeNull();
  });
});
